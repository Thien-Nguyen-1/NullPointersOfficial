...
vi.mock('react-router-dom', async () => {
  ...
  return {
    ...
    Link: ({ to, children }) => (
      <a href={to} onClick={() => mockNavigate(to)}>
        {children}
      </a>
    ),
  };
...
describe('VerifyAdminEmail', () => {
  ...
  //   act(() => {
  //   vi.advanceTimersByTime(10000); // simulate countdown
  // });
  ...
  it('shows default message if no succsesful message displayed', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText((text) =>
        text.includes('Redirecting to login page')
      )).toBeInTheDocument();
  ...
  it('goes to login paeg when "Go to Login" button is clicked', async () => {
    ...
    act(() => {
      screen.getByRole('link', { name: /go to login/i }).click();
    });
    expect(mockNavigate).toHaveBeenCalledWith('/login');
  });
